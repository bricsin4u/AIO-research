...
def sign_content(html_path):
    """Sign the markdown content in an HTML file."""
    check_dependencies()
    
    print(f"\nSigning: {html_path}")
    
    content, soup = extract_markdown_content(html_path)
    if content is None:
        print("  Error: No markdown shadow found")
        return
    ...
    payload = create_signature_payload(content, timestamp)
    signature = private_key.sign(payload.encode('utf-8'))
    signature_b64 = base64.b64encode(signature).decode('ascii')
    
    # Get public key for embedding
    public_b64 = base64.b64encode(public_key.public_bytes(
        encoding=serialization.Encoding.Raw,
        format=serialization.PublicFormat.Raw
    )).decode('ascii')
    ...
    updates = [
        ('aio-truth-signature', signature_b64),
        ('aio-content-hash', content_hash),
        ('aio-public-key', public_b64),
        ('aio-last-verified', timestamp),
        ('aio-signature-algorithm', 'Ed25519'),
    ]
    
    missing_tags = []

    for name, value in updates:
        # Regex to match meta tag with name and content attributes in any order
        # Pattern 1: name then content
        pattern1 = rf'(<meta\s+[^>]*name=["\']{name}["\'][^>]*content=["\'])[^"\']*(["\'][^>]*>)'
        # Pattern 2: content then name
        pattern2 = rf'(<meta\s+[^>]*content=["\'])[^"\']*(["\'][^>]*name=["\']{name}["\'][^>]*>)'
        
        # Try replacing pattern 1
        html_content, count1 = re.subn(pattern1, rf'\g<1>{value}\g<2>', html_content)
        
        if count1 == 0:
            # Try replacing pattern 2
            html_content, count2 = re.subn(pattern2, rf'\g<1>{value}\g<2>', html_content)
            
            if count2 == 0:
                 # Meta tag doesn't exist, queue for addition
                 missing_tags.append(f'<meta name="{name}" content="{value}">')
            
    # Insert missing meta tags into <head>
    if missing_tags:
        new_meta_block = "\n    " + "\n    ".join(missing_tags)
        if '<head>' in html_content:
            html_content = html_content.replace('<head>', f'<head>{new_meta_block}')
            print(f"  Added {len(missing_tags)} missing meta tags")
        else:
            print("  Warning: Could not insert meta tags (no <head> tag found)")

    # Ensure <link rel="ai-instructions"> exists
    if '<link rel="ai-instructions"' not in html_content:
        # Try to insert it after <head>
        if '<head>' in html_content:
            html_content = html_content.replace('<head>', '<head>\n    <link rel="ai-instructions" href="/.well-known/ai-instructions.json">')
            print("  Added <link rel=\"ai-instructions\"> tag")
        else:
             print("  Warning: Could not insert <link rel=\"ai-instructions\"> (no <head> tag found)")
...
def verify_content(html_path, public_key_source=None):
    """Verify the signature of markdown content in an HTML file."""
    check_dependencies()
    ...
    content = md_block.string.strip()
    
    # Extract signature metadata
    sig_meta = soup.find('meta', attrs={'name': 'aio-truth-signature'})
    timestamp_meta = soup.find('meta', attrs={'name': 'aio-last-verified'})
    pubkey_meta = soup.find('meta', attrs={'name': 'aio-public-key'})
    
    if not sig_meta:
        print("  Error: No signature found in file")
        return False
    
    signature_b64 = sig_meta.get('content', '')
    timestamp = timestamp_meta.get('content', '') if timestamp_meta else None
    
    # Get public key
    if public_key_source:
        public_key = load_public_key(public_key_source)
    elif pubkey_meta:
        public_key = load_public_key(pubkey_meta.get('content', ''))
    else:
    ...
    try:
        ...
        print(f"  The content is authentic and has not been tampered with.")
        return True
        
    except InvalidSignature:
        print(f"  Status:     FAILED ✗")
        print(f"  WARNING: Signature verification failed!")
        print(f"  The content may have been tampered with or signed by a different key.")
        return False
